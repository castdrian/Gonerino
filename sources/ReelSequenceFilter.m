...
static NSMapTable *SequenceSnapshots(void)
{
    ...
    dispatch_once(&onceToken, ^{ snapshots = [NSMapTable weakToStrongObjectsMapTable]; });
    return snapshots;
}
...
static NSHashTable *SequenceControllers(void)
{
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{ controllers = [NSHashTable weakObjectsHashTable]; });
    return controllers;
}
...
static NSMapTable *SequenceControllerDataSources(void)
{
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{ dataSources = [NSMapTable weakToStrongObjectsMapTable]; });
    return dataSources;
}
...
static BOOL SnapshotMatches(ReelSequenceSnapshot *snapshot, NSOrderedSet *sourceReels,
                            NSUInteger generation, BOOL filteringEnabled)
{
    id firstSourceReel = sourceReels.count > 0 ? [sourceReels objectAtIndex:0] : nil;
    id lastSourceReel =
        sourceReels.count > 0 ? [sourceReels objectAtIndex:sourceReels.count - 1] : nil;
    return snapshot && snapshot.sourceCount == sourceReels.count &&
           snapshot.firstSourceReel == firstSourceReel &&
           snapshot.lastSourceReel == lastSourceReel && snapshot.generation == generation &&
...
static FeedMetadataRecord *MetadataForReel(id reel)
{
    ...
    NSString           *videoID =
        metadata.videoID.length > 0 ? metadata.videoID : [Util feedVideoIDForObject:reel];
    FeedMetadataRecord *cached =
        videoID.length > 0 ? [Util cachedFeedVideoMetadataForVideoID:videoID] : nil;
    if (!metadata)
        return cached;
    if (!cached)
        return metadata;
    return [[FeedMetadataRecord alloc]
        initWithVideoID:metadata.videoID.length > 0 ? metadata.videoID : cached.videoID
...
static ReelSequenceSnapshot *BuildSnapshot(id dataSource, NSOrderedSet *sourceReels)
{
    ...
    NSMutableArray<NSNumber *> *sourceIndexes =
        [NSMutableArray arrayWithCapacity:sourceReels.count];
    NSMutableArray<FeedMetadataRecord *> *metadata =
        [NSMutableArray arrayWithCapacity:sourceReels.count];
    ...
    for (NSUInteger index = 0; index < sourceReels.count; index++)
    {
        ...
        BOOL                blocked = filteringEnabled && record &&
                                      [Util nodeContainsBlockedVideo:[NSNull null] metadata:record];
        if (blocked)
        {
        ...
        [metadata addObject:record
                                ?: [[FeedMetadataRecord alloc] initWithVideoID:nil
                                                                         title:nil
                                                                       channel:nil]];
    }
...
static void RefreshSequenceController(id controller)
{
    @try
    {
        ...
        {
            currentIndex =
                ((NSUInteger (*)(id, SEL)) objc_msgSend)(controller, @selector(currentReelIndex));
            hasCurrentIndex = YES;
        }
        id pageController = ReelObjectIvar(controller, @"_scrollablePageViewController")
                                ?: ReelObjectIvar(controller, @"_pageViewController");
        id pageAdapter    = ReelObjectIvar(controller, @"_scrollablePageViewControllerAdapter");
        if (!pageAdapter)
            pageAdapter = ReelObjectIvar(pageController, @"_dataSourceAdapter")
                              ?: ReelObjectIvar(pageController, @"_dataSource");
        NSUInteger targetIndex           = hasCurrentIndex && filteredReels.count > 0
                                               ? MIN(currentIndex, filteredReels.count - 1)
                                               : 0;
            ...
            [pageController respondsToSelector:@selector(setDataSourceAdapter:initialIndex:)])
        {
            ((void (*)(id, SEL, id, NSInteger)) objc_msgSend)(
                pageController, @selector(setDataSourceAdapter:initialIndex:), pageAdapter,
        ...
        else if (!reboundPageController &&
                 [controller respondsToSelector:@selector(reloadForSectionsWithGroup:)])
        {
            ((BOOL (*)(id, SEL, id)) objc_msgSend)(controller,
                                                   @selector(reloadForSectionsWithGroup:), nil);
        }
        else if (!reboundPageController &&
                 [controller respondsToSelector:@selector(reloadPreviousAndNextViewControllers)])
        {
            ((void (*)(id, SEL)) objc_msgSend)(controller,
                                               @selector(reloadPreviousAndNextViewControllers));
        }
    }
    @catch (__unused NSException *exception)
    {
...
+ (void)initialize
{
    ...
    [[NSNotificationCenter defaultCenter]
        addObserverForName:FeedFilterStateDidChangeNotification
                    object:nil
                     queue:nil
                usingBlock:^(__unused NSNotification *notification) { [self invalidateAll]; }];
}

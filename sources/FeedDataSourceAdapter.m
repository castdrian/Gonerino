...
@property (nonatomic, strong)
    NSMutableDictionary<NSString *, FeedMetadataRecord *> *metadataBySourcePath;
@property (nonatomic, strong)
    NSMutableDictionary<NSString *, FeedMetadataRecord *>                 *metadataByIdentifier;
@property (nonatomic, strong) NSMutableDictionary<NSString *, NSString *> *sourceIdentityByPath;
@property (nonatomic, strong)
    NSMutableDictionary<NSString *, FeedMetadataRecord *> *forcedBlockedMetadataBySourcePath;
...
- (nullable NSIndexPath *)sourceIndexPathForContentView:(UIView *)contentView
...
- (nullable NSString *)sourceIdentifierAtIndexPath:(NSIndexPath *)indexPath
                                    collectionNode:(nullable id)collectionNode;
...
static NSHashTable<FeedDataSourceAdapter *> *FeedAdapters(void)
{
    ...
    dispatch_once(&onceToken, ^{ adapters = [NSHashTable weakObjectsHashTable]; });
    return adapters;
}
...
static FeedMetadataRecord *MergedFeedMetadata(FeedMetadataRecord *primary,
                                              FeedMetadataRecord *secondary)
{
    if (!primary)
        return secondary;
    if (!secondary)
    ...
    return [[FeedMetadataRecord alloc]
        initWithVideoID:primary.videoID.length > 0 ? primary.videoID : secondary.videoID
...
static FeedMetadataRecord *MetadataWithIdentifier(NSString           *identifier,
...
static void
StoreIdentifierMetadata(NSMutableDictionary<NSString *, FeedMetadataRecord *> *metadataByIdentifier,
                        NSString *identifier, FeedMetadataRecord *metadata)
{
    if (identifier.length == 0 || !metadata)
        return;
...
static void
StoreSourcePathMetadata(NSMutableDictionary<NSString *, FeedMetadataRecord *> *metadataBySourcePath,
                        NSString *sourcePath, FeedMetadataRecord *metadata)
{
...
static void StoreSourceIdentity(NSMutableDictionary<NSString *, NSString *> *sourceIdentityByPath,
                                NSString *sourcePath, NSString *identity)
{
...
static BOOL FeedMetadataMatches(FeedMetadataRecord *candidate, FeedMetadataRecord *changed)
{
    ...
    if (changed.videoID.length > 0 && [candidate.videoID isEqualToString:changed.videoID])
        return YES;
    if (changed.channel.length > 0 &&
        [candidate.channel caseInsensitiveCompare:changed.channel] == NSOrderedSame)
        return YES;
    if (changed.title.length > 0 &&
        [candidate.title localizedCaseInsensitiveContainsString:changed.title])
        return YES;
    return NO;
}
...
static Class EmptyFeedNodeClass(void)
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        if (!emptyNodeClass)
            return;
        Method sizeMethod =
            class_getInstanceMethod(cellNodeClass, @selector(calculateSizeThatFits:));
        const char *typeEncoding =
            sizeMethod ? method_getTypeEncoding(sizeMethod) : "{CGSize=dd}@:{CGSize=dd}";
        class_addMethod(emptyNodeClass, @selector(calculateSizeThatFits:),
                        (IMP) EmptyFeedNodeCalculateSizeThatFits, typeEncoding);
...
static id EmptyFeedNode(void)
{
    ...
    if ([node respondsToSelector:@selector(style)])
    {
        ...
        if ([style respondsToSelector:@selector(setPreferredSize:)])
            ((void (*)(id, SEL, CGSize)) objc_msgSend)(style, @selector(setPreferredSize:),
                                                       CGSizeZero);
    }
    ...
    [[NSNotificationCenter defaultCenter]
        addObserverForName:FeedFilterStateDidChangeNotification
                ...
                usingBlock:^(NSNotification *notification) {
                    FeedMetadataRecord *metadata =
                        [notification.object isKindOfClass:[FeedMetadataRecord class]]
                            ? notification.object
                            : nil;
...
+ (instancetype)adapterWithCollectionView:(UICollectionView *)collectionView
...
+ (void)rememberMetadata:(FeedMetadataRecord *)metadata
    forVisibleItemInCollectionView:(UICollectionView *)collectionView
{
        ...
        inCollectionView:(UICollectionView *)collectionView
{
    ...
    {
        if (adapter.collectionView == collectionView)
            [adapter rememberMetadata:metadata
...
+ (void)rememberMetadata:(FeedMetadataRecord *)metadata forContentView:(UIView *)contentView
{
    ...
    for (FeedDataSourceAdapter *adapter in adapters)
    {
        ...
        if (!collectionView.isPagingEnabled && ![className containsString:@"short"] &&
...
+ (FeedMetadataRecord *)cachedMetadataForContentView:(UIView *)contentView
                                    inCollectionView:(UICollectionView *)collectionView
{
    ...
    for (FeedDataSourceAdapter *adapter in adapters)
    {
        if (adapter.collectionView != collectionView)
            continue;
        NSIndexPath *sourceIndexPath = [adapter sourceIndexPathForContentView:contentView
        ...
        {
            FeedMetadataRecord *metadata =
                adapter.metadataBySourcePath[FeedSourcePathKey(sourceIndexPath)];
            if (metadata)
                return metadata;
        }
    }
...
- (BOOL)respondsToSelector:(SEL)selector
{
    ...
    if (selector == @selector(collectionView:numberOfItemsInSection:))
        return
            ...
            [self.dataSource respondsToSelector:@selector(collectionNode:numberOfItemsInSection:)];
    ...
    if (selector == @selector(numberOfSectionsInCollectionView:))
        return [self.dataSource respondsToSelector:selector] ||
               [self.dataSource respondsToSelector:@selector(numberOfSectionsInCollectionNode:)];
    ...
    if (selector == @selector(modelIdentifierForElementAtIndexPath:inNode:) ||
        ...
        selector ==
            @selector(modelIndexPathForSupplementaryElementOfKind:atPresentationIndexPath:) ||
        selector ==
            @selector(reportItemWillBecomeVisibleWithModelIndexPath:presentationIndexPath:) ||
...
- (NSMethodSignature *)methodSignatureForSelector:(SEL)selector
{
    return [super methodSignatureForSelector:selector]
               ?: [self.dataSource methodSignatureForSelector:selector];
}
...
- (id)collectionNodeObject
{
    ...
    return [collectionView respondsToSelector:@selector(collectionNode)]
               ? collectionView.collectionNode
               : nil;
}
    ...
    @try
    {
        if (collectionNode &&
            ...
            return ((NSInteger (*)(id, SEL, id)) objc_msgSend)(
                self.dataSource, @selector(numberOfSectionsInCollectionNode:), collectionNode);
        if ([self.dataSource respondsToSelector:@selector(numberOfSectionsInCollectionView:)])
            return ((NSInteger (*)(id, SEL, id)) objc_msgSend)(
                self.dataSource, @selector(numberOfSectionsInCollectionView:), self.collectionView);
    }
    ...
    @try
    {
        if (collectionNode &&
            ...
            return ((NSInteger (*)(id, SEL, id, NSInteger)) objc_msgSend)(
                self.dataSource, @selector(collectionNode:numberOfItemsInSection:), collectionNode,
                section);
        if ([self.dataSource respondsToSelector:@selector(collectionView:numberOfItemsInSection:)])
            return ((NSInteger (*)(id, SEL, id, NSInteger)) objc_msgSend)(
                self.dataSource, @selector(collectionView:numberOfItemsInSection:),
...
{
    if (![self.dataSource
            respondsToSelector:@selector(modelIdentifierForElementAtIndexPath:inNode:)])
        return nil;
    @try
    {
        id identifier = ((id (*)(id, SEL, id, id)) objc_msgSend)(
            self.dataSource, @selector(modelIdentifierForElementAtIndexPath:inNode:), indexPath,
...
- (FeedMetadataRecord *)knownMetadataAtSourceIndexPath:(NSIndexPath *)indexPath
                                                 model:(id)model
                                            identifier:(NSString *)identifier
{
    BOOL                identifierIsVideoID = IsLikelyVideoIdentifier(identifier);
    FeedMetadataRecord *metadata = identifierIsVideoID
                                       ? [[FeedMetadataRecord alloc] initWithVideoID:identifier
                                                                               title:nil
                                                                             channel:nil]
                                       : nil;
    @synchronized(self)
    {
        ...
        FeedMetadataRecord *stableMetadata =
            identifierIsVideoID ? self.metadataByIdentifier[identifier] : nil;
    ...
    if (metadata.dictionaryRepresentation.count > 0)
    {
        @synchronized(self)
        {
            StoreSourcePathMetadata(self.metadataBySourcePath, FeedSourcePathKey(indexPath),
                                    metadata);
            if (identifierIsVideoID)
...
- (NSArray<NSNumber *> *)sourceCounts
{
    ...
    NSMutableArray<NSNumber *> *counts =
        [NSMutableArray arrayWithCapacity:(NSUInteger) sectionCount];
...
- (FeedCollectionSnapshot *)snapshotForCountRequestWithRetryCount:(NSUInteger)retryCount
{
    ...
    for (NSUInteger attempt = retryCount; attempt < 4; attempt++)
    {
        ...
        NSMutableArray<NSArray<NSNumber *> *> *sourceItemsBySection =
            [NSMutableArray arrayWithCapacity:counts.count];
        NSMutableArray<NSDictionary<NSNumber *, NSNumber *> *> *visibleItemsBySection =
            [NSMutableArray arrayWithCapacity:counts.count];
        ...
        for (NSUInteger section = 0; section < counts.count; section++)
        {
            ...
            NSMutableArray<NSNumber *> *sourceItems =
                [NSMutableArray arrayWithCapacity:(NSUInteger) sourceCount];
            NSMutableDictionary<NSNumber *, NSNumber *> *visibleItems =
                [NSMutableDictionary dictionaryWithCapacity:(NSUInteger) sourceCount];
            for (NSInteger sourceItem = 0; sourceItem < sourceCount; sourceItem++)
            {
                if (filteringEnabled)
                {
                    NSString *sourcePath =
                        FeedSourcePathKeyForItem((NSInteger) section, sourceItem);
                    ...
                    metadata = MergedFeedMetadata(
                        metadata, [Util cachedFeedVideoMetadataForVideoID:identifier]);
...
- (FeedCollectionSnapshot *)currentSnapshot
{
    ...
    if (!snapshot || snapshot.generation != generation ||
        snapshot.sourceGeneration != sourceGeneration ||
        snapshot.filteringEnabled != [self filteringEnabledForAdapter])
        return nil;
    return snapshot;
}
...
- (NSIndexPath *)sourceIndexPathForVisibleIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section < 0 ||
        indexPath.section >= (NSInteger) snapshot.sourceItemsBySection.count)
        return nil;
    ...
    if (indexPath.item < 0 || indexPath.item >= (NSInteger) items.count)
        return nil;
    return [NSIndexPath indexPathForItem:items[(NSUInteger) indexPath.item].integerValue
...
- (NSIndexPath *)visibleIndexPathForSourceIndexPath:(NSIndexPath *)indexPath
{
    FeedCollectionSnapshot *snapshot = [self currentSnapshot] ?: [self snapshotForCountRequest];
    if (indexPath.section < 0 ||
        indexPath.section >= (NSInteger) snapshot.visibleItemsBySection.count)
        return nil;
    NSNumber *visibleItem =
        snapshot.visibleItemsBySection[(NSUInteger) indexPath.section][@(indexPath.item)];
    return visibleItem
               ? [NSIndexPath indexPathForItem:visibleItem.integerValue inSection:indexPath.section]
               : nil;
}

- (void)rememberNode:(id)node
    atSourceIndexPath:(NSIndexPath *)indexPath
           identifier:(NSString *)identifier
{
    if (!node || !indexPath)
        return;
    ...
    NSString *sourceIdentity =
        [identifier isKindOfClass:[NSString class]] && identifier.length > 0 ? identifier : nil;
    ...
    FeedMetadataRecord *knownMetadata = [self knownMetadataAtSourceIndexPath:indexPath
                                                                       model:nil
                                                                  identifier:effectiveIdentifier];
    FeedMetadataRecord *nodeMetadata =
        identifierIsVideoID ? MetadataWithIdentifier(identifier, cachedMetadata) : cachedMetadata;
    ...
    @synchronized(self)
    {
        ...
        if (sourceIdentity.length > 0)
            StoreSourceIdentity(self.sourceIdentityByPath, FeedSourcePathKey(indexPath),
                                sourceIdentity);
        else
...
- (void)rememberMetadata:(FeedMetadataRecord *)metadata forNode:(id)node
{
    ...
    if (!self.performingFilteredReload && [Util nodeContainsBlockedVideo:[NSNull null]
                                                                metadata:mergedMetadata])
        [self queueFilteredReload];
}

- (void)storeMetadata:(FeedMetadataRecord *)metadata
    atSourceIndexPath:(NSIndexPath *)sourceIndexPath
{
...
- (void)rememberMetadata:(FeedMetadataRecord *)metadata
    forVisibleItemInCollectionView:(UICollectionView *)collectionView
{
    if (!metadata || metadata.dictionaryRepresentation.count == 0 ||
        self.collectionView != collectionView)
        return;

    NSArray<NSIndexPath *> *visibleIndexPaths = [collectionView.indexPathsForVisibleItems
        sortedArrayUsingComparator:^NSComparisonResult(NSIndexPath *first, NSIndexPath *second) {
    ...
    {
        CGPoint center =
            CGPointMake(CGRectGetMidX(collectionView.bounds), CGRectGetMidY(collectionView.bounds));
...
- (NSIndexPath *)sourceIndexPathForContentView:(UIView *)contentView
                                collectionView:(UICollectionView *)collectionView
{
    ...
    for (NSUInteger depth = 0; view && depth < 16; depth++, view = view.superview)
    {
        ...
        NSIndexPath *visibleIndexPath =
            [collectionView indexPathForCell:(UICollectionViewCell *) view];
        return visibleIndexPath ? [self sourceIndexPathForVisibleIndexPath:visibleIndexPath] : nil;
    }

    if ([collectionView respondsToSelector:@selector(indexPathForItemAtPoint:)])
    {
        CGPoint center = [collectionView convertPoint:CGPointMake(CGRectGetMidX(contentView.bounds),
        ...
        inCollectionView:(UICollectionView *)collectionView
{
    ...
    NSIndexPath *sourceIndexPath = [self sourceIndexPathForContentView:contentView
                                                        collectionView:collectionView];
    if (!sourceIndexPath)
    {
        NSString *className = NSStringFromClass([collectionView class]).lowercaseString;
     ...
     numberOfItemsInSection:(NSInteger)section
{
    ...
    return section >= 0 && section < (NSInteger) snapshot.sourceItemsBySection.count
               ? (NSInteger) snapshot.sourceItemsBySection[(NSUInteger) section].count
               : 0;
}
    ...
    nodeForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!sourceIndexPath)
        return ((id (*)(id, SEL, id, id)) objc_msgSend)(
            self.dataSource, @selector(collectionView:nodeForItemAtIndexPath:), collectionView,
            indexPath);
    id node = ((id (*)(id, SEL, id, id)) objc_msgSend)(
        self.dataSource, @selector(collectionView:nodeForItemAtIndexPath:), collectionView,
        sourceIndexPath);
...
- (FeedNodeBlock)collectionView:(UICollectionView *)collectionView
    nodeBlockForItemAtIndexPath:(NSIndexPath *)indexPath
{
    NSIndexPath *sourceIndexPath = [self sourceIndexPathForVisibleIndexPath:indexPath];
    if (!sourceIndexPath)
        return ((id (*)(id, SEL, id, id)) objc_msgSend)(
            self.dataSource, @selector(collectionView:nodeBlockForItemAtIndexPath:), collectionView,
            indexPath);
    FeedNodeBlock block = ((id (*)(id, SEL, id, id)) objc_msgSend)(
        self.dataSource, @selector(collectionView:nodeBlockForItemAtIndexPath:), collectionView,
        sourceIndexPath);
...
- (id)collectionNode:(id)collectionNode nodeForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!sourceIndexPath)
        return ((id (*)(id, SEL, id, id)) objc_msgSend)(
            self.dataSource, @selector(collectionNode:nodeForItemAtIndexPath:), collectionNode,
            indexPath);
    id node = ((id (*)(id, SEL, id, id)) objc_msgSend)(
        self.dataSource, @selector(collectionNode:nodeForItemAtIndexPath:), collectionNode,
        sourceIndexPath);
...
- (FeedNodeBlock)collectionNode:(id)collectionNode
    nodeBlockForItemAtIndexPath:(NSIndexPath *)indexPath
{
    NSIndexPath *sourceIndexPath = [self sourceIndexPathForVisibleIndexPath:indexPath];
    if (!sourceIndexPath)
        return ((id (*)(id, SEL, id, id)) objc_msgSend)(
            self.dataSource, @selector(collectionNode:nodeBlockForItemAtIndexPath:), collectionNode,
            indexPath);
    FeedNodeBlock block = ((id (*)(id, SEL, id, id)) objc_msgSend)(
        self.dataSource, @selector(collectionNode:nodeBlockForItemAtIndexPath:), collectionNode,
        sourceIndexPath);
...
- (id)collectionNode:(id)collectionNode nodeModelForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!sourceIndexPath)
        return ((id (*)(id, SEL, id, id)) objc_msgSend)(
            self.dataSource, @selector(collectionNode:nodeModelForItemAtIndexPath:), collectionNode,
            indexPath);
    ...
    FeedMetadataRecord *metadata   = [self knownMetadataAtSourceIndexPath:sourceIndexPath
                                                                    model:model
                                            ...
                                            inNode:(id)collectionNode
{
    ...
    {
        return ((id (*)(id, SEL, id, id)) objc_msgSend)(
            self.dataSource, @selector(modelIdentifierForElementAtIndexPath:inNode:),
            sourceIndexPath, collectionNode);
    }
    @catch (__unused NSException *exception)
    {
        return nil;
                                                 ...
                                                 inNode:(id)collectionNode
{
    @try
    {
        NSIndexPath *sourceIndexPath = ((id (*)(id, SEL, id, id)) objc_msgSend)(
            self.dataSource, @selector(indexPathForElementWithModelIdentifier:inNode:), identifier,
            collectionNode);
...
- (NSIndexPath *)presentationIndexPathForModelIndexPath:(NSIndexPath *)modelIndexPath
{
    ...
    if ([self.dataSource respondsToSelector:_cmd])
    {
        ...
        {
            upstreamIndexPath =
                ((id (*)(id, SEL, id)) objc_msgSend)(self.dataSource, _cmd, sourceIndexPath);
        }
        @catch (__unused NSException *exception)
        {
    ...
    NSIndexPath *visibleIndexPath =
        [self visibleIndexPathForSourceIndexPath:upstreamIndexPath ?: sourceIndexPath];
...
- (NSIndexPath *)modelIndexPathForPresentationIndexPath:(NSIndexPath *)presentationIndexPath
{
    ...
    @try
    {
        return ((id (*)(id, SEL, id)) objc_msgSend)(self.dataSource, _cmd, sourceIndexPath)
                   ?: sourceIndexPath;
    }
    @catch (__unused NSException *exception)
    {
...
{
    NSIndexPath *sourcePresentationIndexPath =
        [self sourceIndexPathForVisibleIndexPath:presentationIndexPath] ?: presentationIndexPath;
    NSIndexPath *sourceModelIndexPath =
        [self modelIndexPathForPresentationIndexPath:presentationIndexPath] ?: modelIndexPath;
    if ([self.dataSource respondsToSelector:_cmd])
        ((void (*)(id, SEL, id, id)) objc_msgSend)(self.dataSource, _cmd, sourceModelIndexPath,
...
- (void)reportItemDidBecomeHiddenWithModelIndexPath:(NSIndexPath *)modelIndexPath
{
    ...
    if ([self.dataSource respondsToSelector:@selector(presentationIndexPathForModelIndexPath:)])
    {
        @try
        {
            NSIndexPath *visibleIndexPath = ((id (*)(id, SEL, id)) objc_msgSend)(
                self.dataSource, @selector(presentationIndexPathForModelIndexPath:),
                modelIndexPath);
            sourceModelIndexPath =
                [self modelIndexPathForPresentationIndexPath:visibleIndexPath] ?: modelIndexPath;
        }
        @catch (__unused NSException *exception)
        {
        }
                                     ...
                                     atPresentationIndexPath:(NSIndexPath *)presentationIndexPath
{
    ...
    @try
    {
        return ((id (*)(id, SEL, id, id)) objc_msgSend)(self.dataSource, _cmd, elementKind,
                                                        sourceIndexPath);
    }
    @catch (__unused NSException *exception)
    {
...
- (FeedMetadataRecord *)metadataForNode:(id)node
{
    ...
    {
        sourcePath = [self.sourcePathByNode objectForKey:node];
        FeedMetadataRecord *sourceMetadata =
            sourcePath.length > 0 ? self.metadataBySourcePath[sourcePath] : nil;
        metadata          = MergedFeedMetadata(sourceMetadata, nodeMetadata);
        NSString *videoID = [Util feedVideoIDForObject:node];
        metadata          = MergedFeedMetadata(
            metadata, videoID.length > 0 ? self.metadataByIdentifier[videoID] : nil);
    }
    return metadata;
}
...
- (void)queueFilteredReload
{
    if (!NSThread.isMainThread)
    {
        dispatch_async(dispatch_get_main_queue(), ^{ [self queueFilteredReload]; });
        return;
    }
    @synchronized(self)
    {
        ...
        @try
        {
            ...
            if ([collectionNode respondsToSelector:@selector(reloadDataWithCompletion:)])
            {
                ((void (*)(id, SEL, id)) objc_msgSend)(
                    collectionNode, @selector(reloadDataWithCompletion:), [finishReload copy]);
            }
...
- (void)finishFilteredReloadWithToken:(NSUInteger)token
{
    if (!NSThread.isMainThread)
    {
        dispatch_async(dispatch_get_main_queue(), ^{ [self finishFilteredReloadWithToken:token]; });
        return;
    }
    BOOL reloadAgain = NO;
    @synchronized(self)

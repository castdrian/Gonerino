...
- (instancetype)init
{
    ...
    if (self)
    {
        NSArray *storedVideos =
            [[NSUserDefaults standardUserDefaults] arrayForKey:@"GonerinoBlockedVideos"];
...
- (void)addBlockedVideo:(NSString *)videoId title:(NSString *)title channel:(NSString *)channel
{
    ...
    NSInteger existingIndex = [self.blockedVideoArray
...
{
    NSIndexSet *indexes = [self.blockedVideoArray

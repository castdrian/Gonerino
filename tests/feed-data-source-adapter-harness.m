...
{
    return section >= 0 && section < self.nodesBySection.count
               ? self.nodesBySection[(NSUInteger) section].count
               : 0;
}
...
{
    FakeNode *node =
        self.nodesBySection[(NSUInteger) indexPath.section][(NSUInteger) indexPath.item];
...
int main(void)
{
    @autoreleasepool
    {
        ...
        FeedDataSourceAdapter *adapter =
            [FeedDataSourceAdapter adapterWithCollectionView:(UICollectionView *) collectionView
                                                  dataSource:dataSource];

        Require([adapter numberOfSectionsInCollectionNode:collectionNode] == 2, @"section count");
        Require([adapter collectionNode:collectionNode numberOfItemsInSection:0] == 3,
                @"initial source count");
        id blockedNode = [adapter collectionNode:collectionNode
                          nodeForItemAtIndexPath:[NSIndexPath indexPathForItem:1 inSection:0]];
        Require([blockedNode isKindOfClass:NSClassFromString(@"FeedEmptyCellNode")],
                @"blocked node was not replaced");
        PumpMainQueue();
        ...
        Require([adapter collectionNode:collectionNode numberOfItemsInSection:0] == 2,
                @"blocked item remained in snapshot");
        Require(collectionNode.reloadCalls == 1, @"late metadata caused more than one reload");
        Require([[adapter modelIdentifierForElementAtIndexPath:[NSIndexPath indexPathForItem:1
                                                                                   inSection:0]
                                                        inNode:collectionNode]
                    isEqualToString:third.identifier],
                @"visible index did not translate to source index");
        Require([adapter indexPathForElementWithModelIdentifier:blocked.identifier
                                                         inNode:collectionNode] == nil,
                @"blocked source identifier remained addressable");
        Require([[adapter indexPathForElementWithModelIdentifier:third.identifier
                                                          inNode:collectionNode]
                    isEqual:[NSIndexPath indexPathForItem:1 inSection:0]],
        ...
        FeedDataSourceAdapter *pagedAdapter = [FeedDataSourceAdapter
            adapterWithCollectionView:(UICollectionView *) pagedCollectionView
                           dataSource:dataSource];
        id pagedBlockedNode = [pagedAdapter collectionNode:pagedCollectionNode
                                    nodeForItemAtIndexPath:[NSIndexPath indexPathForItem:1
                                                                               inSection:0]];
        ...
        FeedNodeBlock nodeBlock = [adapter collectionNode:collectionNode
                              nodeBlockForItemAtIndexPath:[NSIndexPath indexPathForItem:1
                                                                              inSection:0]];
        ...
        FeedDataSourceAdapter    *directAdapter        = [FeedDataSourceAdapter
            adapterWithCollectionView:(UICollectionView *) directCollectionView
                           dataSource:dataSource];
        Require([directAdapter
                    numberOfSectionsInCollectionView:(UICollectionView *) directCollectionView] ==
                    2,
                @"direct collection-view section count");
        Require([directAdapter collectionView:(UICollectionView *) directCollectionView
                       numberOfItemsInSection:0] == 3,
        ...
        FeedNodeBlock directNodeBlock =
            [directAdapter collectionView:(UICollectionView *) directCollectionView
                nodeBlockForItemAtIndexPath:[NSIndexPath indexPathForItem:1 inSection:0]];
        Require(
            [[directNodeBlock() class] isSubclassOfClass:NSClassFromString(@"FeedEmptyCellNode")],
        ...
        NSOrderedSet   *sourceReels =
            [NSOrderedSet orderedSetWithObjects:first, blocked, third, unknown, nil];
        NSOrderedSet *filteredReels = [ReelSequenceFilter filteredReelsForDataSource:reelDataSource
                                                                         sourceReels:sourceReels];
        Require(filteredReels.count == 3, @"shorts snapshot count");
        Require([filteredReels objectAtIndex:0] == first &&
                    [filteredReels objectAtIndex:1] == third &&
                    [filteredReels objectAtIndex:2] == unknown,
                @"shorts snapshot order");
        ...
        Require([ReelSequenceFilter visibleIndexForObject:blocked
                                               dataSource:reelDataSource] == NSNotFound,
                @"blocked shorts object remained addressable");
        Require([ReelSequenceFilter visibleIndexForVideoID:third.identifier
                                                dataSource:reelDataSource] == 1,
                @"shorts video identifier did not translate to visible index");
        NSSet *sourceVideoIDs =
            [NSSet setWithObjects:first.identifier, blocked.identifier, third.identifier, nil];
        NSSet *filteredVideoIDs = [ReelSequenceFilter filteredVideoIDsForDataSource:reelDataSource
                                                                     sourceVideoIDs:sourceVideoIDs];
        Require([filteredVideoIDs containsObject:first.identifier] &&
                    ![filteredVideoIDs containsObject:blocked.identifier],
        ...
        filteredReels = [ReelSequenceFilter filteredReelsForDataSource:reelDataSource
                                                           sourceReels:sourceReels];
        ...
        NSLog(@"PASS: adapter snapshot, index translation, zero-node fallback, stale block checks, "
              @"and reel sequence filtering");
    }
    return 0;
}
